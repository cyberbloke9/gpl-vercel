...
export const ReportsViewer = () => {
  ...
  const getReportTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'emergency': return 'bg-destructive/10 text-destructive border-destructive';
  ...
  const handleGenerateReport = async (type: 'daily' | 'weekly' | 'monthly'): Promise<void> => {
    try {
      ...
      // Fetch completed checklists with full details (exclude emergencies for regular reports)
  ...
  const handleDownloadReport = async (report: Report): Promise<void> => {
    try {
      ...
      const isEmergencyReport = report.report_type === 'emergency';
      ...
      const { data: completedChecklists, error: checklistsError } = await supabase
        ...
        `)
        .eq('is_emergency', isEmergencyReport)
    ...
    <div className="space-y-6 p-4">
      <div className="flex flex-col gap-4">
        ...
        <p className="text-muted-foreground text-sm">
          Reports are automatically generated after completing all 6 checklists in a session. Emergency checklists are tracked separately.
        </p>
      </div>
      ...
      {reports.length === 0 ? (
        ...
        <div className="space-y-8">
          ...
          {/* Emergency Reports */}
          {getFilteredReports('emergency').length > 0 && (
            <div>
              <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
              ...
              <div className="space-y-4">
                {getFilteredReports('emergency').map((report) => (
                  <Card key={report.id} className="hover:shadow-lg transition-shadow border-destructive/50">
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                        ...
                        {report.summary && (
                          <div className="mt-2 space-y-1">
                            <div className="flex gap-4">
                              <span>✓ {report.summary.totalChecklists || 0} emergency checklists</span>
                              <span>⚠️ {report.summary.totalIssues || 0} issues</span>
                            </div>
                            {report.summary.emergencyReason && (
                              <p className="text-destructive font-medium">Reason: {report.summary.emergencyReason}</p>
                            )}
                          </div>
                        )}
                      </div>
